...
function Blog() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`https://live-server-bh9y.onrender.com/api/posts?showAll=true`);
                setPosts(res.data);
            } catch (err) {
                console.log(err);
            }
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(`https://live-server-bh9y.onrender.com/api/posts/${postToDelete.id}`);
    ...
    const handleFileDelete = async (fileUrl) => {
        try {
            await axios.delete('https://live-server-bh9y.onrender.com/api/delete', {
                data: { url: fileUrl }
            });
        } catch (error) {
